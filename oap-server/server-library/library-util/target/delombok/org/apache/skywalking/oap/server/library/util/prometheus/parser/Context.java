...
public class Context {
    ...
    void end() {
        ...
        switch (type) {
        case GAUGE: 
            samples.forEach(textSample -> metricFamilyBuilder.addMetric(Gauge.builder().name(name).value(convertStringToDouble(textSample.getValue())).labels(textSample.getLabels()).timestamp(now).build()));
            break;
        case COUNTER: 
            samples.forEach(textSample -> metricFamilyBuilder.addMetric(Counter.builder().name(name).value(convertStringToDouble(textSample.getValue())).labels(textSample.getLabels()).timestamp(now).build()));
            break;
        case HISTOGRAM: 
            samples.stream().map(sample -> {
                ...
                samples.forEach(textSample -> {
                    if (textSample.getName().endsWith("_count")) {
                        ...
                        hBuilder.bucket(convertStringToDouble(textSample.getLabels().remove("le")), (long) convertStringToDouble(textSample.getValue()));
                    }
                });
            ...
            samples.stream().map(sample -> {
                ...
                samples.forEach(textSample -> {
                    if (textSample.getName().endsWith("_count")) {
                        ...
                        sBuilder.quantile(convertStringToDouble(textSample.getLabels().remove("quantile")), convertStringToDouble(textSample.getValue()));
                    }
                });
