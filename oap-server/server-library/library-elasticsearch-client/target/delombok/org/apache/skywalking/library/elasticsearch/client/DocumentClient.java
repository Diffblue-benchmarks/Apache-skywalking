...
public final class DocumentClient {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(DocumentClient.class);
    ...
    public boolean exists(String index, String type, String id) {
        try {
            return version.thenCompose(v -> client.execute(v.requestFactory().document().exist(index, type, id)).aggregate().thenApply(response -> response.status() == HttpStatus.OK).exceptionally(e -> {
    ...
    public Optional<Document> get(String index, String type, String id) {
        try {
            final CompletableFuture<Optional<Document>> future = version.thenCompose(v -> client.execute(v.requestFactory().document().get(index, type, id)).aggregate().thenApply(response -> {
    ...
    public Optional<Documents> mget(String type, Map<String, List<String>> indexIds) {
        try {
            final CompletableFuture<Optional<Documents>> future = version.thenCompose(v -> client.execute(v.requestFactory().document().mget(type, indexIds)).aggregate().thenApply(response -> {
                if (response.status() != HttpStatus.OK) {
                    throw new RuntimeException(response.contentUtf8());
                }
                try (
    ...
    public void index(IndexRequest request, Map<String, Object> params) {
        try {
            final CompletableFuture<Void> future = version.thenCompose(v -> client.execute(v.requestFactory().document().index(request, params)).aggregate().thenAccept(response -> {
    ...
    public void update(UpdateRequest request, Map<String, Object> params) {
        try {
            final CompletableFuture<Void> future = version.thenCompose(v -> client.execute(v.requestFactory().document().update(request, params)).aggregate().thenAccept(response -> {
                final HttpStatus status = response.status();
    ...
    public void deleteById(String index, String type, String id, Map<String, Object> params) {
        try {
            final CompletableFuture<Void> future = version.thenCompose(v -> client.execute(v.requestFactory().document().deleteById(index, type, id, params)).aggregate().thenAccept(response -> {
                final HttpStatus status = response.status();
                if (status != HttpStatus.OK) {
                    throw new RuntimeException(response.contentUtf8());
                }
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public DocumentClient(final CompletableFuture<ElasticSearchVersion> version, final WebClient client) {
