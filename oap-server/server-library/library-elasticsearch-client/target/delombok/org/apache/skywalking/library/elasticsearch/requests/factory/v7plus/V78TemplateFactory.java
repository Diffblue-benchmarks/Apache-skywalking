...
final class V78TemplateFactory implements TemplateFactory {
    ...
    public HttpRequest exists(String name) {
        return HttpRequest.builder().get("/_index_template/{name}").pathParam("name", name).build();
    }
    ...
    public HttpRequest delete(final String name) {
        return HttpRequest.builder().delete("/_index_template/{name}").pathParam("name", name).build();
    }
    ...
    public HttpRequest createOrUpdate(String name, Map<String, ?> settings, Mappings mappings, int order) {
        try {
            ...
            final Map<String, Object> template = ImmutableMap.<String, Object>builder().put("index_patterns", patterns).put("template", ImmutableMap.builder().put("aliases", aliases).put("settings", settings).put("mappings", mappings).build()).build();
            final byte[] content = version.codec().encode(template);
            return HttpRequest.builder().put("/_index_template/{name}").pathParam("name", name).content(MediaType.JSON, content).build();
    ...
    @lombok.Generated
    public V78TemplateFactory(final ElasticSearchVersion version) {
