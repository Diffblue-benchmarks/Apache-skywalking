...
public final class ElasticSearch implements Closeable {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ElasticSearch.class);
    ...
    ElasticSearch(SessionProtocol protocol, String username, String password, EndpointGroup endpointGroup, ClientFactory clientFactory, Consumer<Boolean> healthyListener, Duration responseTimeout) {
        ...
        final WebClientBuilder builder = WebClient.builder(protocol, endpointGroup).factory(clientFactory).responseTimeout(responseTimeout).decorator(LoggingClient.builder().logger(log).newDecorator()).decorator(RetryingClient.builder(RetryRule.failsafe()).maxTotalAttempts(3).newDecorator());
    ...
    public CompletableFuture<ElasticSearchVersion> connect() {
        final CompletableFuture<ElasticSearchVersion> future = client.get("/").aggregate().thenApply(response -> {
            final HttpStatus status = response.status();
            if (status != HttpStatus.OK) {
                throw new RuntimeException("Failed to connect to ElasticSearch server: " + response.contentUtf8());
            }
            try (
        ...
        future.whenComplete((v, throwable) -> {
            if (throwable != null) {
                final RuntimeException cause = new RuntimeException("Failed to determine ElasticSearch version", throwable);
    ...
    public SearchResponse scroll(Duration contextRetention, String scrollId) {
        return searchClient.scroll(Scroll.builder().contextRetention(contextRetention).scrollId(scrollId).build());
    }
