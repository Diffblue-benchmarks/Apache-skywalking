...
public final class TemplateClient {
    ...
    @lombok.Generated
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(TemplateClient.class);
    private final CompletableFuture<ElasticSearchVersion> version;
    private final WebClient client;

    public boolean exists(String name) {
        try {
            final CompletableFuture<Boolean> future = version.thenCompose(v -> client.execute(v.requestFactory().template().exists(name)).aggregate().thenApply(response -> {
    ...
    public Optional<IndexTemplate> get(String name) {
        try {
            final CompletableFuture<Optional<IndexTemplate>> future = version.thenCompose(v -> client.execute(v.requestFactory().template().get(name)).aggregate().thenApply(response -> {
                final HttpStatus status = response.status();
                ...
                try (
                    ...
                    final IndexTemplates templates = v.codec().decode(is, IndexTemplates.class);
    ...
    public boolean delete(String name) {
        try {
            final CompletableFuture<Boolean> future = version.thenCompose(v -> client.execute(v.requestFactory().template().delete(name)).aggregate().thenApply(response -> {
    ...
    public boolean createOrUpdate(String name, Map<String, Object> settings, Mappings mappings, int order) {
        try {
            final CompletableFuture<Boolean> future = version.thenCompose(v -> client.execute(v.requestFactory().template().createOrUpdate(name, settings, mappings, order)).aggregate().thenApply(response -> {
                if (response.status() == HttpStatus.OK) {
                    return true;
                }
                throw new RuntimeException(response.contentUtf8());
    ...
    @java.lang.SuppressWarnings("all")
    @lombok.Generated
    public TemplateClient(final CompletableFuture<ElasticSearchVersion> version, final WebClient client) {
