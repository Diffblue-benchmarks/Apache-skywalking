...
final class V6IndexFactory implements IndexFactory {
    ...
    public HttpRequest putMapping(String index, String type, Mappings mapping) {
        try {
            ...
            return HttpRequest.builder().put("/{index}/_mapping/{type}").pathParam("index", index).pathParam("type", type).content(MediaType.JSON, content).build();
        } catch (final java.lang.Throwable $ex) {
            throw lombok.Lombok.sneakyThrow($ex);
        }
    }
    ...
    @lombok.Generated
    public V6IndexFactory(final ElasticSearchVersion version) {
