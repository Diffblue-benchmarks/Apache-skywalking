...
final class V6TemplateFactory implements TemplateFactory {
    ...
    public HttpRequest exists(String name) {
        return HttpRequest.builder().get("/_template/{name}").pathParam("name", name).build();
    }
    ...
    public HttpRequest delete(final String name) {
        return HttpRequest.builder().delete("/_template/{name}").pathParam("name", name).build();
    }
    ...
    public HttpRequest createOrUpdate(String name, Map<String, ?> settings, Mappings mappings, int order) {
        try {
            ...
            final Map<String, Object> template = ImmutableMap.<String, Object>builder().put("index_patterns", patterns).put("aliases", aliases).put("settings", settings).put("mappings", mappings).put("order", order).build();
            final byte[] content = version.codec().encode(template);
            return HttpRequest.builder().put("/_template/{name}").pathParam("name", name).content(MediaType.JSON, content).build();
    ...
    @lombok.Generated
    public V6TemplateFactory(final ElasticSearchVersion version) {
